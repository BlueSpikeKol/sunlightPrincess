...
namespace PlatformerPro
{
	...
	{
        private static string[] s_cachedTaggedPropertyNames;
        private static string[] CachedTaggedPropertyNames
        {
            get
            {
                if(s_cachedTaggedPropertyNames == null)
                {
                    s_cachedTaggedPropertyNames = GetTaggedPropertyNames();
                }

                return s_cachedTaggedPropertyNames;
            }
        }
		...
		public override void OnInspectorGUI()
		{
			...
			if (myTarget.actions != null)
			{
				for (int i = 0; i < myTarget.actions.Length; i++)
				{
					...
					if (!removed) RenderAction(myTarget, myTarget, myTarget.actions[i]);
		...
		public static void RenderAction(object target, object repsonder, EventResponse action)
		{
			...
			if (action.responseType == EventResponseType.LOAD_SCENE) {
                ...
                if(scenes.Length > 0) action.message = scenes[index];
			}
        ...
        /// <summary>
        /// Gets all scene names in the build settings
        /// </summary>
        protected static string[] GetBuildSettingsSceneNames()
        {
            ...
            {
                scenes[j] = System.IO.Path.GetFileNameWithoutExtension(UnityEngine.SceneManagement.SceneUtility.GetScenePathByBuildIndex(j));
            }
