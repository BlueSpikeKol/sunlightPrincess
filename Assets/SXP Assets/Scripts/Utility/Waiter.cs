...
public class Waiter : MonoBehaviour
{
    ...
    public Waiter ThenWaitUntil(Func<float, bool> cond)
    {
        Task t = Waiters.taskCache.Take();
        t.Reset(cond);
...
{
    internal static readonly PoolableItem<Task> taskCache = new PoolableItem<Task>(() => new Task());
    ...
    public static Waiter WaitUntil(Func<float, bool> condition, GameObject attachTo = null)
    {
        Waiter w = (attachTo ?? GlobalWaiter).AddComponent<Waiter>();
        return w.ThenWaitUntil(condition);
    }
...
internal class Task
{
    ...
    internal void Reset(Func<float, bool> condition, Action<float> onTick = null)
    {
        this.timeWaited = 0;
        this.onCleanup = null;
